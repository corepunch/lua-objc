	...
	for (NSDictionary *item in [_items arrayByAddingObjectsFromArray:_pageItems ?: @[]]) {
		if ([toolbar_item_identifier(item[@"id"]) isEqualToString:identifier]) {
			...
			NSToolbarItem *ti = ([item[@"type"] isEqualToString:@"search"]
				|| [content isKindOfClass:NSSearchField.class])
			...
			ti.autovalidates = NO;
			ti.enabled = YES;
			// AppKit chooses the glass grouping when the item is inserted. Both
			// the border policy and custom content must be configured beforehand.
			...
			if ([ti isKindOfClass:NSSearchToolbarItem.class]) {
				if (content) {
					NSSearchToolbarItem *search = (NSSearchToolbarItem *)ti;
					search.searchField = (NSSearchField *)content;
					NSNumber *width = objc_getAssociatedObject(content, &kKeys[kFixedWidthKey]);
					if (width) search.preferredWidthForSearchField = width.doubleValue;
				}
				return ti;
			}
