...
if ([obj isKindOfClass:[NSWindow class]]) {
	if (strcmp(key, "size") == 0) {
		push_NSSize(L, ((NSWindow *)obj).contentView.frame.size);
		return 1;
	}
}
if ([obj isKindOfClass:[NSView class]]) {
	if (strcmp(key, "size") == 0) {
		push_NSSize(L, ((NSView *)obj).frame.size);
		return 1;
	}
	if (strcmp(key, "frameInWindow") == 0) {
		push_NSRect(L, [((NSView *)obj) convertRect:((NSView *)obj).bounds toView:nil]);
		return 1;
	}
}
...
if ([obj isKindOfClass:[NSWindow class]]) {
	if (strcmp(key, "size") == 0) {
		((NSWindow *)obj).contentSize = *check_NSSize(L, 3);
		return 0;
	}
}
if ([obj isKindOfClass:[NSView class]]) {
	if (strcmp(key, "size") == 0) {
		((NSView *)obj).frameSize = *check_NSSize(L, 3);
		return 0;
	}
}
