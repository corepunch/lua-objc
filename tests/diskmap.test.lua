...
local config = xml.renderFile("apps/diskmap/views/Window.etlua", {windowTitle = "Diskmap", subtitle = "1 TB free", actions = {search = function() end}}, ns)
t.assertEqual(config.width, 1100, "default window fits sidebar, chart and legend")
t.assertEqual(config.minWidth, 880, "category columns remain usable at minimum width")
t.assertEqual(config.sidebarWidth, 210, "navigation uses a native sidebar")
...
t.assertEqual(sidebar.rowCount, 7, "sidebar lists sections and destinations")
...
t.expect(not bridge._tableCell(ui.refs.results, 1, 0).levelIndicator.hidden, "measured categories show a share bar")
local _, loadingIds = Inventory.plan(ui.model)
Inventory.begin(ui.model, loadingIds); ui:updateRows()
sizeCell = bridge._tableCell(ui.refs.results, 2, 0)
...
t.expect(bridge._tableCell(ui.refs.results, 2, 0).loadingIndicator.hidden, "cancel removes category spinner")
