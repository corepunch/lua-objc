...
t.assertEqual(config.width, 800, "narrow default window width")
t.assertEqual(config.height, 720, "default window height")
t.assertEqual(config.minWidth, 720, "category list remains usable at minimum width")
...
ui:updateRows()
t.assertEqual(ui.capacity.text, ui.categories:capacity(ui.scan.disk), "scan updates continue on Reclaim")
local filteredReclaim = ui.cleanup:presentation("DerivedData")
t.assertEqual(#filteredReclaim.groups[1].rows, 1, "Reclaim search finds a matching measured candidate")
t.assertEqual(#ui.cleanup:presentation("no match").groups[1].rows, 0, "Reclaim search can show an empty group")
...
ui.navigation:selectRow(0)
t.assertEqual(ui.section, "Storage", "Storage can be revisited from Reclaim")
t.assertEqual(ui.refs.opportunities, nil, "returning to Storage does not remount reclaim suggestions")
t.assertEqual(ui.refs.coveragePanel.frame.size.width, ui.refs.categoriesPane.frame.size.width, "scan quality and categories use one content column")
