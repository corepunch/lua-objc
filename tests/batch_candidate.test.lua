...
t.assertEqual(p.root.y, 0)
t.assertEqual(p.root.width, 300)
t.assertEqual(p.root.height, 200)
t.assertEqual(p.first.x, 105, "native default horizontal centering")
t.assertEqual(p.first.y, 11, "top-left despite unflipped native root")
t.assertEqual(p.row.y, 38)
...
t.assertEqual(larger.probes[1].height, 400)
t.assertEqual(result.probes[1].height, 200, "subsequent cases do not mutate results")
...
t.assertEqual(ns._parityReadJSON(output .. "/model.json").probes[1].height, 30)
