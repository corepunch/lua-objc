local function testWebPage()
    ...
    local findResult
    assert(attached:find("Local", { backwards = true, caseSensitive = true }, function(found)
        findResult = found
    end))
    assert(calls[#calls][1] == "find" and calls[#calls][2] == "Local")
    assert(calls[#calls][3] == true and calls[#calls][4] == true and calls[#calls][5] == true)
    calls[#calls][6](true)
    assert(findResult == true)
    attached:setPageZoom(1.5)
    assert(attached.pageZoom == 1.5 and calls[#calls][1] == "setPageZoom")
    assert(calls[#calls][2] == 1.5 and observed.pageZoom == 1.5)
    attached:setMagnification(1.25)
    assert(calls[#calls][1] == "setMagnification" and calls[#calls][2] == 1.25)
    local absent = WebPage.new("about:blank")
    assert(absent:find("missing", function(found, err)
        assert(found == false and err:find("not attached"))
    end) == false)
    local validZoom = pcall(function() attached:setPageZoom(0) end)
    assert(not validZoom and attached.pageZoom == 1.5)
