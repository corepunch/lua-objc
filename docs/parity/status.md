...
- Deterministic `demo/swiftui_parity/` gallery added with the three P1
...
UIKit `Picker` now maps `segmented` to `UISegmentedControl`, `menu` and
...
`Text`/`Label` now accept `systemImage` and compose a native SF Symbol view with
