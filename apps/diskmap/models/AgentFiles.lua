...
function AgentFiles.add(model, entries)
	local paths = {}; for _, row in ipairs(model.resources:leaves()) do if row.path then paths[row.path] = true end end
	for _, entry in ipairs(entries or {}) do
		...
		if parent and not parent:isLeaf() and entry.path and not paths[entry.path] then
			...
			if kind then
				...
				local added, err = model.resources:add(parent.id, row)
