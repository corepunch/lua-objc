...
local Model = require("apps.container-reorder.Model")
...
function Controller:createWindow()
	local root, refs = xml.renderFile("apps/container-reorder/views/Root.etlua", {}, ns)
	self.window = ns.Window { title = "Reorder Containers", width = 560,
		height = 640, content = root }
	self.content = Template.new(refs.host, "apps/container-reorder/views/Content.etlua", ns)
	self:render()
	return self.window
end
